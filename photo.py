...
class ImageRecognition(TaskSet):
	...
	def picture(self):
		try:
			...
			exit(0)
		#open操作不能在put_nowait里操作，要不然文件会一直打开，上传的图片大小为0
		files={'file':(request_file,open(request_file,'rb'),'image/jpg')}
		...
		with self.client.post("/test/cucaInvoice/invoice/uploadPicture",headers=headers,files=files,catch_response=True) as response:
